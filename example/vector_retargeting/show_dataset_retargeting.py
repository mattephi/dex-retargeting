...
# VR has 26 joints, MediaPipe has 21
...
# Coordinate transform from VR FLU (Forward-Left-Up) to MANO convention
...
def load_dataset(dataset_path: str) -> dict:
    """Load VR hand tracking dataset from JSON file."""
    with open(dataset_path, 'r') as f:
        return json.load(f)


def extract_joint_positions(frame: dict, hand_type: str) -> Optional[np.ndarray]:
    """Extract and transform joint positions from a dataset frame.

    Args:
        frame: A single frame from the dataset
        hand_type: "Left" or "Right"

    Returns:
        Joint positions in MediaPipe format (21, 3) or None if hand not tracked
    """
    hand_key = "left_hand" if hand_type == "Left" else "right_hand"
    hand_data = frame.get(hand_key)

    if hand_data is None or not hand_data.get("is_tracked", False):
        return None

    positions = np.array(hand_data["positions"])  # (26, 3)
...
def main(
    ...
    """
    Plays back hand poses from a VR dataset and retargets them to a robot hand.

    Args:
        robot_name: The identifier for the robot. This should match one of the default supported robots.
        retargeting_type: The type of retargeting, each type corresponds to a different retargeting algorithm.
        hand_type: Specifies which hand is being tracked, either left or right.
        dataset_path: Path to the JSON dataset file. Defaults to squeeze.json in current directory.
        playback_speed: Speed multiplier for playback. 1.0 = realtime, 2.0 = 2x speed.
        loop: Whether to loop the dataset playback.
    """
    if dataset_path is None:
        dataset_path = str(Path(__file__).parent / "squeeze.json")
    ...
    logger.info(f"Loaded {metadata['frame_count']} frames, duration: {metadata['duration_seconds']:.2f}s")
    ...
    while True:
        ...
        for i, frame in enumerate(frames):
            ...
            joint_pos = extract_joint_positions(frame, hand_type_str)

            if joint_pos is None:
                logger.warning(f"Frame {i}: {hand_type_str} hand is not tracked.")
            else:
            ...
            # Timing control based on dataset timestamps
            if i + 1 < len(frames):
                target_dt = (frames[i + 1]["timestamp_seconds"] - frame["timestamp_seconds"]) / playback_speed
